...
- **v54 — "Voir le logement" du menu chat ne faisait rien** : avant, clic sur le bouton affichait juste un toast `Logement : <titre>`. Pino : "le bouton voir le logement ne fait rien quand on clique dessus !". Fix v54 (incomplet) : ajout du prop `onOpenListing(conv)` propagé app.js → MessagesScreen → ChatScreen + résolution item par listingId/titre.
- **v55 — Hotfix v54 : conflit popstate entre messages.js et app.js** : Pino confirme cas (c) : "Le chat se ferme mais aucune fiche détail ne s'ouvre". Diagnostic : v54 appelait `window.history.back()` dans le wrapper avant `setDetail(item)`, ce qui déclenchait le popstate listener de **ByerApp** (lignes 497-504 de app.js) qui appelle `closeAllOverlays()` → reset de `detail` à `null` immédiatement après le set. Fix v55 : retiré `history.back()` du wrapper. On appelle juste `setDetail(item)` ; ByerApp gère automatiquement la pile d'historique overlay via `useEffect [onSecondaryScreen]` qui pushState `_byerOverlay`. MessagesScreen unmount visuellement quand DetailScreen prend la place. Quand l'utilisateur revient (back arrow ou phone back), il retombe sur la liste des conversations (chat fermé naturellement par unmount). **Leçon** : ne jamais introduire un 2e système history.pushState/popstate sans vérifier les interactions avec celui qui existe déjà.

- **v56 — ErrorBoundary message technique visible par défaut** : avant, le détail d'erreur était dans un `<details>` collapsé. L'utilisateur non-tech ne pensait pas à l'ouvrir → impossible de diagnostiquer les crashes signalés. Fix : remplacement par un `<div>` visible avec le message tech encadré et un titre "📋 Message technique (à copier-coller à l'équipe)". Le bug Dashboard signalé par Pino post-v55 a disparu après hard reload (probablement résiduel cache SW byer-v55).

- **v57 — Lift openChat à ByerApp + closeTopOverlay (back stack chat → detail → return to chat)** : Pino : "lorsqu'on clique sur le bouton retour on revienne dans la page precedente". Avant : MessagesScreen unmount quand DetailScreen prend la place → state local `openChat` perdu → au retour, conv list affichée au lieu de la conv elle-même. Refactor :
  1. **State `messagesOpenChat` lifté à ByerApp** : survit au unmount/remount de MessagesScreen.
  2. **closeTopOverlayRef** : remplace `closeAllOverlays` dans le popstate handler. Ferme l'overlay du dessus selon priorité visuelle (detail → ... → messagesOpenChat). Permet la nav stack "chat → detail → back ferme detail → chat encore visible".
  3. **overlayDepth + per-overlay pushState** : remplace l'ancien push unique (`if (!cur._byerOverlay) push`). Chaque ouverture d'overlay/chat incrémente la profondeur et push une entry dédiée → chaque back en pop une.
