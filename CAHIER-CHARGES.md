...
- **v54 — "Voir le logement" du menu chat ne faisait rien** : avant, clic sur le bouton affichait juste un toast `Logement : <titre>`. Pino : "le bouton voir le logement ne fait rien quand on clique dessus !". Fix : ajout du prop `onOpenListing(conv)` propagé app.js → MessagesScreen → ChatScreen. App.js résout l'item listing depuis (1) `conv.listingId` (Supabase via embed `c.listing_id` ajouté dans l'adapter) ou (2) match par titre `conv.logement` dans `dbListings ∪ PROPERTIES ∪ VEHICLES`. Si trouvé → `setDetail(item)` + `history.back()` pour fermer le chat. Si non trouvé → fallback toast (silencieux pour conversation orpheline).
